...
final class CryptoUtil {
    ...
    static final byte[] BUCKET_NAME_MAGIC = Hex.decode("398734aab3c4c30c9f22590e83a95f7e43556a45fc2b3060e0c39fde31f50272");
    static final byte[] BUCKET_META_MAGIC = Hex.decode("42964710327258a0a3239a41a2d5e2d7468a393d3413d2aa26a4a2c856c90251");
    ...
    static byte[] hmacSha512(final byte[] key, final byte[] input) {
    ...
    static byte[] hmacSha512Half(final byte[] key, final byte[] input) {
        byte[] decryptKey = hmacSha512(key, input);
        return Arrays.copyOfRange(decryptKey, 0, decryptKey.length / 2);
    }

    static byte[] ripemd160(final byte[] input) {
    ...
    static byte[] ripemd160Sha256(final byte[] input) throws NoSuchAlgorithmException {
        byte[] sha256bytes = sha256(input);
        return ripemd160(sha256bytes);
    }

//    static byte[] ripemd160Sha256Double(final byte[] input) throws NoSuchAlgorithmException {
//        return ripemd160Sha256(ripemd160Sha256(input));
//    }

    static String ripemd160Sha256HexString(final byte[] input) throws NoSuchAlgorithmException {
        return Hex.toHexString(ripemd160Sha256(input));
    }

//    static String ripemd160Sha256HexStringDouble(final byte[] input) throws NoSuchAlgorithmException {
//        return Hex.toHexString(ripemd160Sha256Double(input));
//    }

    static byte[] generateDeterministicKey(final byte[] key, final byte[] id ) throws NoSuchAlgorithmException {
        byte[] sha512input = ArrayUtils.addAll(key, id);
        return Arrays.copyOfRange(sha512(sha512input), 0, 32);
    }

    static byte[] generateGenaroSeed(byte[] key) {
        PKCS5S2ParametersGenerator gen = new PKCS5S2ParametersGenerator(new SHA512Digest());
        gen.init(key, "mnemonic".getBytes(UTF_8), SEED_ITERATIONS);
        return ((KeyParameter) gen.generateDerivedParameters(SEED_KEY_SIZE)).getKey();
    }

    static byte[] generateBucketKey(final byte[] privKey, final byte[] bucketId) throws NoSuchAlgorithmException {
        byte[] seed = generateGenaroSeed(privKey);
        return generateDeterministicKey(seed, bucketId);
    }

    static byte[] generateFileKey(final byte[] privKey, final byte[] bucketId, final byte[] index) throws NoSuchAlgorithmException {
        byte[] bKey = generateBucketKey(privKey, bucketId);
        return generateDeterministicKey(bKey, index);
    }

    static String encryptMeta(final byte[] fileMeta, final byte[] encryptKey, final byte[] encryptIv)
    ...
    static String encryptMetaHmacSha512(byte[] meta, byte[] privKey, byte[] bucketId)
            throws NoSuchAlgorithmException, NoSuchPaddingException, BadPaddingException, IllegalBlockSizeException,
            InvalidKeyException, NoSuchProviderException, InvalidAlgorithmParameterException {
    ...
    static String decryptMetaHmacSha512(String bufferBase64, byte[] privKey, byte[] bucketId)
    ...
    static String sha256EscdaSign(final BigInteger ecPrivateKey, final String message) throws NoSuchAlgorithmException {
        return sha256EscdaSign(ecPrivateKey, BasicUtil.string2Bytes(message));
    }

    private static String sha256EscdaSign(final BigInteger ecPrivateKey, final byte[] message) throws NoSuchAlgorithmException {
